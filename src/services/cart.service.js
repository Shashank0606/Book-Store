...
export const addedToCart = async (EmailId, params_book_id) => {
  try {
    ...
    const existingBook = userCart.books.find(
      (book) => book.productId === params_book_id
    );
    if (existingBook) {
    ...
    const updatedCart = await cart.findOneAndUpdate(
      { userId: EmailId },
      { books: userCart.books, cart_total: totalPrice },
...
export const removeBookFromCart = async (EmailId, params_book_id) => {
  ...
  if (checkCart) {
    ...
    const updatedCart = await cart.findOneAndUpdate(
      { userId: EmailId },
      { books: checkCart.books, cart_total: totalPrice },
      { new: true }
    );
    return updatedCart;
  } else {
