...
export const addBook = async (EmailId, params_book_id) => {
    try {
        ...
        const userCart = await Wishlist.findOne({ userId: EmailId }) || { userId: EmailId, books: [] };
        ...
        const updatedCart = await Wishlist.findOneAndUpdate({ userId: EmailId }, { books: userCart.books }, { new: true, upsert: true });
        return updatedCart;
    } catch (error) {
        console.error(error);
...
export const removeBook = async (EmailId, params_book_id) => {
    const checkCart = await Wishlist.findOne({ userId: EmailId });
    if (checkCart) {
        ...
        const updatedCart = await Wishlist.findOneAndUpdate({ userId: EmailId }, { books: checkCart.books, cart_total: totalPrice }, { new: true })
        return updatedCart
    } else {
